...
export default function FeaturedCollection({
  ...
  const [fetched, setFetched] = useState<Collection[] | null>(null);

  /* The server list is the source of truth (including when it's empty);
     the API fallback is only for renders that got no server data. */
  const collections = initialCollections ?? fetched ?? [];
  ...
  /* No active collections — render nothing at all so the home page
     doesn't keep an empty gap where the section used to be. */
